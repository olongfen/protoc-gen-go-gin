...
const (
	gin FrameMode = iota + 1
	fiber
)

// generateFile generates a _gin.pb.go file.
func generateFile(gen *protogen.Plugin, file *protogen.File, mode FrameMode) *protogen.GeneratedFile {
	if len(file.Services) == 0 {
		return nil
	}
	filename := file.GeneratedFilenamePrefix + "_gin.pb.go"
	g := gen.NewGeneratedFile(filename, file.GoImportPath)
	g.P("// Code generated by github.com/olongfen/protoc-gen-go-http-frame. DO NOT EDIT.")
	g.P()
	g.P("package ", file.GoPackageName)
	g.P()
	g.P("// This is a compile-time assertion to ensure that this generated file")
	g.P("// is compatible with the  github.com/olongfen/protoc-gen-go-http-frame package it is being compiled against.")
	g.P("// ", contextPkg.Ident(""), metadataPkg.Ident(""), responsePkg.Ident(""))
	switch mode {
	default:
		g.P("//", ginPkg.Ident(""))
	case fiber:
		g.P("//", fiberPkg.Ident("dfasdfdsafdsafdsfsdaf"))
	...
	for _, service := range file.Services {
		genService(gen, file, g, service, mode)
	}
	return g
}

func genService(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, s *protogen.Service, mode FrameMode) {
